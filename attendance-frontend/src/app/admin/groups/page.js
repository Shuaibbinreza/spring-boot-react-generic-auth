  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-maroon-200 pb-4">
        <h1 className="text-2xl font-bold text-maroon-700 tracking-tight">Group Management</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-maroon-300 p-6 shadow-maroon-sm lg:col-span-1">
          ...
          <form onSubmit={handleCreateGroup} className="space-y-4">
            <div>
              <label htmlFor="groupName" className="block text-xs font-semibold text-stone-700 uppercase tracking-wider mb-1.5">
                Group Name
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="groupDesc" className="block text-xs font-semibold text-stone-700 uppercase tracking-wider mb-1.5">
                Description
              </label>
              <textarea
                id="groupDesc"
        ...
        <div className="bg-white border border-maroon-300 p-6 shadow-maroon-sm lg:col-span-2">
          ...
          {loading ? (
            ...
            <p className="text-sm text-stone-500 py-6 text-center">No groups created yet. Create a group above.</p>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div>
                      ...
                      <p className="text-xs text-stone-600 mt-0.5">{group.description || 'No description provided'}</p>
                    </div>
                    <span className="px-2.5 py-1 text-xs font-semibold bg-stone-100 text-stone-700 border border-stone-200 whitespace-nowrap">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      className="px-3 py-1 text-xs font-semibold bg-maroon-50 text-maroon-700 border border-maroon-300 hover:bg-maroon-200 transition-colors cursor-pointer"
        ...
        <div className="bg-white border border-maroon-300 p-6 shadow-maroon-sm">
          ...
          <form onSubmit={handleAddMember} className="flex gap-3 mb-6">
            <select
              ...
              required
              className="flex-1 px-3.5 py-2 bg-white border border-maroon-300 text-stone-900 text-sm focus:outline-none focus:border-maroon-700"
            >
            ...
            <button
              ...
              className="px-4 py-2 bg-maroon-700 hover:bg-maroon-800 text-white font-semibold text-xs whitespace-nowrap disabled:opacity-50 cursor-pointer transition-colors"
            >
          ...
          <div>
            ...
            {!activeGroup.members || activeGroup.members.length === 0 ? (
              <p className="text-sm text-stone-500 py-4 text-center">No members in this group yet.</p>
            ) : (
                  ...
                  <tbody className="divide-y divide-maroon-100">
                    {activeGroup.members.map((member) => (
                      <tr key={member.id} className="hover:bg-maroon-100/50 transition-colors">
                        ...
                        <td className="py-2.5 px-3 text-stone-700">{member.email}</td>
                        <td className="py-2.5 px-3">
                          <button
                            className="px-2.5 py-1 text-xs font-medium bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 transition-colors cursor-pointer"
